...
class TripSegment(object):
    ...
    def __init__(self, enter_connection: Connection, exit_connection: Connection):
        self.id = enter_connection.trip_id
    ...
    def __repr__(self):
        return f'<TripSegment ({self.id}, {self.enter_connection.dep_stop} -> {self.exit_connection.arr_stop})>'

    def __str__(self):
        s = f'TripSegment of trip {self.id}'
